...
| Chrome (PC)        | ✅ 完全支持 | ✅ 支持  |
| Edge (PC)          | ✅ 完全支持 | ✅ 支持  |
| Chrome (Android)   | ✅ 完全支持 | ❌ 不支持 |
| Firefox            | ✅ 完全支持 | ❌ 不支持 |
| Safari (iOS/macOS) | ✅ 完全支持 | ❌ 不支持 |

### 历史记录功能限制

历史记录功能依赖 **File System Access API** 来持久化保存文件句柄，但该 API 有明显的平台限制：

**为什么移动端浏览器不支持：**

1. **Android Chrome**：虽然 API 技术上存在，但实际行为不稳定：
   - 权限请求可能静默失败
   - 存储的句柄在浏览器重启后经常失效
   - 文件访问变得不一致
2. **iOS Safari**：完全不支持 File System Access API
3. **Firefox/Safari (桌面版)**：不支持 File System Access API

**为什么 OPFS（源私有文件系统）不是解决方案：**

- OPFS 是虚拟文件系统，可以存储文件，但配额有限（约100-500MB）
- 漫画文件通常 30-200MB，存储几个就会超出配额
- 复制大文件到 OPFS 很慢，影响用户体验
- iOS Safari 也不支持 OPFS

因此，历史记录功能仅在 **PC Chrome/Edge** 上可用，不支持的平台会自动隐藏该按钮。
